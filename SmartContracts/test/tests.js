...
{  
    ...
    it("The call of getTimeOfMint(uint256) was not successful (getDuration, unpauseMembership, returnURI)", async function ()
    {
        const {NFTFitness, AppNFT, owner, user} = await loadFixture(deploy);
        await NFTFitness.connect(owner).mint('tariff', 5, 2);
        await NFTFitness.connect(owner).setContractConnect(AppNFT.getAddress());
        ...
        await expect(AppNFT.connect(owner).returnURI(1, 1)).to.be.revertedWith("The call of getTimeOfMint(uint256) was not successful");
    });
